...
- The Candidates page can optionally use a generated `Cinematic Kill Shot`. This is a real HLAE campath, not a third-person chase camera or DemoUI mouse macro: the parser exports generation-safe attacker/victim tracks, the planner begins one second before the primary kill, frames both players, holds the victim briefly, and retains `camera_plan.json` plus `camera_path.xml` with the recording session.
- POV cinematics require the victim to remain networked continuously through the pre-kill window. The planner requests a one-second lead but automatically shortens it to the longest shared continuous attacker/victim segment. The diagnostic test build requires only three real pre-kill ticks—enough for distinct camera keys—rather than imposing an aesthetic duration threshold. PVS, removal, respawn, and identity gaps are never bridged or extrapolated. BSP/VPK collision checking is intentionally disabled, so missing map geometry cannot reject a cinematic; generated paths may pass through walls while tracking is evaluated. An unavailable track reports the candidate and full nested rejection reason, then offers Original Camera or Cancel.

...
- POV cinematics require the victim to remain networked continuously through the pre-kill window. PVS, removal, respawn, and identity gaps are rejected rather than extrapolated. BSP/VPK collision checking is intentionally disabled in this victim-tracking test build, so missing map geometry cannot reject a cinematic; generated paths may pass through walls while tracking is evaluated. An unavailable track offers Original Camera or Cancel and is never silently treated as a successful cinematic.

...
- POV cinematics require the victim to remain networked continuously through the pre-kill window. PVS, removal, respawn, and identity gaps are rejected rather than extrapolated. The planner reads the exact matching BSP from `tf/maps` or TF2's installed VPK archives; camera endpoints, swept motion, and subject sightlines are tested against solid map brushes. An unavailable angle offers Original Camera or Cancel and is never silently treated as a successful cinematic.

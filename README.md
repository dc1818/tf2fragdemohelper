...
- Candidate scoring includes the legacy frag identifiers plus bookmark identifiers. Every bookmark creates a candidate, adds its bookmark score, and inherits nearby frag identifiers when present.
...
- Preview stages a temporary demo and VDM seek script and applies the same per-kill camera plan used by recording.
- Every selected kill—including a single-kill candidate—offers `Original / Attacker`, `Victim Focus`, and `Outside / Cinematic`. Multi-kill candidates can mix camera styles kill-by-kill or apply one style to all selected kills. STV victim views use spectator targeting; POV victim/outside views use a world-space HLAE camera derived from the captured attacker/victim positions. Non-original views hold briefly after impact, then restore the source-appropriate attacker/POV view.
...
- Recording completion uses explicit per-clip and final-batch markers. Process monitoring tolerates transient Windows query failures, confirms TF2 exit across repeated polls, bounds HLAE shutdown, and preserves completed clips without waiting 20 seconds for every untouched queue entry after an interruption.

...
- Manual HLAE launch stages the selected demo, seeks to the current `Before first tick` time, retains the `After last tick` boundary for naming/reference, and installs temporary number-row (`1` through `=`, including `0` and `-`) camera, kill-tick, campath, and recording hotkeys. `4` cycles through every distinct kill tick, including multi-tick candidates.

...
- The Windows package also includes `TF2_MIRV_Director.exe`. A manual session opens it beside TF2 with the selected clip window, exact cue ticks, per-tick tags, available victim names, whole-candidate tags, and output/campath paths. It is a separate companion process and closes with the TF2 session. See [TF2 MIRV Director game plan](docs/TF2_MIRV_DIRECTOR_GAMEPLAN.md) for the researched HOT-inspired architecture and live-control roadmap.

...
- Preview stages a temporary demo and VDM seek script. Recording uses the original POV view, or the established attacker in-eye spectator focus for STV candidates.
- The Candidates page can optionally use a generated `Cinematic Kill Shot`. This is a real HLAE campath, not a third-person chase camera or DemoUI mouse macro: the parser exports generation-safe attacker/victim tracks, and the planner generates an ordered victim-led shot for every distinct kill demo tick. Simultaneous victims share a shot and are framed together. Each involved track is retained from the candidate clip start, the victim is followed through impact, and a brief hold precedes the transition to the next kill. The recording session retains `camera_plan.json` plus `camera_path.xml` with per-shot tick and victim diagnostics.
- POV cinematics require each victim to remain continuously networked through that victim's segment. The first shot starts from the candidate lead-in when coverage permits; later shots begin after the preceding hold, with shorter transitions for rapid kills. Three real pre-kill ticks are preferred for distinct camera keys, while rapid later kills still receive their own impact shot. PVS, removal, respawn, and identity gaps are never bridged or extrapolated. BSP/VPK collision checking is intentionally disabled, so missing map geometry cannot reject a cinematic; generated paths may pass through walls while tracking is evaluated. An unavailable track reports the candidate and full nested rejection reason, then offers Original Camera or Cancel.
- Camera selection defaults to `Original Camera` on every application launch. Selecting Cinematic Kill Shot applies to the current session and is not silently restored on the next launch.
- HLAE receives the documented POV setup order (`sv_cheats 1`, `thirdperson`, `mirv_input camera`), loads the generated XML, executes `mirv_input end`, and only then enables `mirv_campath`. Existing candidate ticks and normal clip timing remain unchanged; camera timing alone uses the demo's exported `interval_per_tick`.

...
- Manual HLAE launch stages the selected demo, seeks to the current `Before first tick` time, retains the `After last tick` boundary for naming/reference, and installs temporary F2-F12 camera, kill-tick, campath, and recording hotkeys. F4 cycles through every distinct kill tick, including multi-tick candidates.
- Manual captures are written under `Manual HLAE/` in the selected recording directory. F9 and F10 use the selected recording format, FPS, encoder, and image quality. F12 saves the current campath as `camera_path.xml` in that capture folder.

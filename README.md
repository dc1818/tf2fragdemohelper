...
- POV cinematics require the victim to remain networked continuously through the pre-kill window. The planner requests a one-second lead but automatically shortens it to the longest shared continuous attacker/victim segment when at least 0.35 seconds remains. PVS, removal, respawn, and identity gaps are never bridged or extrapolated. BSP/VPK collision checking is intentionally disabled in this victim-tracking test build, so missing map geometry cannot reject a cinematic; generated paths may pass through walls while tracking is evaluated. An unavailable track reports the candidate and full nested rejection reason, then offers Original Camera or Cancel.
